...
# 1.反向查找,查询host表中c1的信息,会得到一个对象,对象中存在一个已经设置好的虚拟关系:h
u = session.query(HostUser).filter(HostUser.username == 'root').first()
print(u)  # <__main__.HostUser object at 0x11096a780>

print(u.host)  # [<__main__.Host object at 0x102ec3860>]

# 2.正向查找,遍历对象属性
for item in u.host:
    print("-" * 20)
    print(item)  # 是HostToHostUser对象
    print(item.hostname)  # <__main__.HostUser object at 0x102b18b38>
    print(item.port)
    print(item.ip)
